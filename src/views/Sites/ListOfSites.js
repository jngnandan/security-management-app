...
export default function ListOfSites() {
      const {employees, posts, sites, loading,} = useContext(ContentContext)
      ...
      const renderClient = async (id) => {
            const findClient = await sites.find(client => client.id === id)
            setSiteDetails(findClient)
      }
      ...
      {siteDetails ? (
        <div className="card shadow w-10/12 card">
            <div className="flex flex-row justify-end">
                <BsFillXCircleFill className='w-8 h-8 m-4 mt-8 text-gray-500 hover:text-gray-900' onClick={() => setSiteDetails(null)} />
            </div>
            <div>
        ...
        <div className='flex flex-row justify-start mb-1'>
            ...
            <div className='flex flex-row mx-4 items-center'>
                ...
                <table className="w-11/12 mr-6">
                    ...
                    {sites.slice(setCurrentClients, currentPage).map((client, index) => (
                        <SiteItem key={index} client={client} renderClient={renderClient}/>
                    ))}
                                {/* Pagination */}
